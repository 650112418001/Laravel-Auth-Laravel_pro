<x-admin-layout>
    ...
    <style>
        .text-lg {
            font-size: 1.2rem; /* ขนาดตัวอักษรที่ใหญ่ขึ้น */
        }

        .font-weight-bold {
            font-weight: 550; /* ทำให้ตัวหนามากขึ้น */
        }

        .card-header {
            background-color: #f8f9fa; /* เปลี่ยนสีพื้นหลังของการ์ด */
            border-bottom: 1px solid #e7e7e7; /* เพิ่มขอบล่าง */
        }

        .progress {
            height: 20px; /* ปรับความสูงของแถบโปรเกรส */
            border-radius: 5px; /* มุมโค้งมน */
        }

        .progress-bar {
            transition: width 0.4s ease; /* เพิ่มเอฟเฟกต์การเปลี่ยน */
            border-radius: 5px; /* มุมโค้งมน */
        }
    </style>
                            ...
                            <div class="card border-left-primary shadow h-100 py-2">
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        ...
                                        <div class="col-auto">
                                            <i class="fas fa-calendar fa-3x text-gray-300"></i> <!-- Icon แสดงเหตุการณ์ -->
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            @foreach($events as $event)
                                <h4 class="small font-weight-bold">{{ $event->title }} 
                                    <span class="float-right">{{ number_format($event->progress, 2) }}%</span>
                                </h4>
                                <div class="progress mb-3"> <!-- ลด margin-bottom ของ progress -->
                                    <div class="progress-bar" role="progressbar" style="width: {{ number_format($event->progress, 2) }}%; background-color: {{ $event->progressClass }};" aria-valuenow="{{ number_format($event->progress, 2) }}" aria-valuemin="0" aria-valuemax="100"></div>
                                </div>
                            @endforeach
                        </div>
                    </div>
                                    ...
                                    <tbody>
                                        @foreach ($attendings as $attending)
                                            <tr>
                                                ...
                                                <td>{{ \Carbon\Carbon::parse($attending->event_time)->format('H:i A') }}</td>
                                                <td>{{ \Carbon\Carbon::parse($attending->event_start)->format('d M Y') }}</td>
                                                <td>{{ \Carbon\Carbon::parse($attending->event_end)->format('d M Y') }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
